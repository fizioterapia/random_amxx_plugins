...
#define DOUBLE_JUMP_FLAG 262144 // ADMIN_LEVEL_G (s), https://wiki.alliedmods.net/Adding_Admins_(AMX_Mod_X)#Access_Levels
    ...
    for(new id = 1; id <= MAX_PLAYERS; id++) {
        if(is_user_connected(id) && g_Jumpers[id] && is_user_alive(id)) {
            if(g_Jumps[id] == 0 && (get_gametime() - g_LastUserJump[id]) > DOUBLE_JUMP_TIME) {
                g_Jumps[id] = 1;
                ColorChat(id, GREEN, "[%s]^x01 You can double jump again!", DOUBLE_JUMP_PREFIX);
            }
        }
    }
}
...
public CmdStartPreJumper(id, uc_handle) {
    ...
    if((get_uc(uc_handle, UC_Buttons) & IN_JUMP) && !(flags & FL_ONGROUND) && !(pev(id, pev_oldbuttons) & IN_JUMP) && g_Jumps[id] > 0) {
        ...
        ColorChat(id, GREEN, "[%s]^x01 Double jump used, wait %.0f seconds.", DOUBLE_JUMP_PREFIX, DOUBLE_JUMP_TIME);
    }
}
